...
```{r Frequency of protective behaviour: Random effects plot}
plotcmm.ranef <- function(cmm1) {
  ...
  ggplot() +
    geom_point(aes(x = 1:nc, y = cmm1$ranef[ord.re])) +  # Points for random effects
    geom_errorbar(aes(x = 1:nc, ymin = ci[,1], ymax = ci[,2]), width = 0.2) +  # Confidence intervals
...
```{r Count of protective behaviours: Random effects plot}
plotcompois.ranef <- function(compois1) {
  rn <- names(compois1[["stDev"]]) 
  nc <- length(compois1[["ranef"]]) 
  ci <- compois1$ranef + qnorm(0.975) * sqrt(compois1$condVar) %o% c(-1, 1)
  ord.re <- order(compois1$ranef)
  ci <- ci[order(compois1$ranef), ]

  ggplot() +
    geom_point(aes(x = 1:nc, y = compois1$ranef[ord.re])) +  # Points for random effects
    geom_errorbar(aes(x = 1:nc, ymin = ci[,1], ymax = ci[,2]), width = 0.2) +  # Confidence intervals
    geom_hline(yintercept = 0, linetype = "dashed") +  # Reference line at 0
    theme_minimal() +
    theme(axis.text.x = element_blank(),  # Remove x-axis labels
