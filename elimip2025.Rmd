...
# Calculate confiance 
df$confiance <- (df$confiance_nat + df$confiance_reg) / 2
df$confiance <- as.numeric(df$confiance)

# Calculate threat
df <- df %>% mutate_at(c("disease_prob", "disease_fear"), as.numeric)
df$threat <- (df$disease_prob + df$disease_fear) / 2

# Calculate place
df$place <- paste(df$region, df$commune_type, sep = " ")
